...
function AdminDrinkDeleteForm({
  ...
  useEffect(() => {
    ...
    if (
      Object.keys(storesState[selectedStoreUUID].storeMenu).length !== 0 &&
      selectedDrinkUUID === ""
    ) {
        ...
        [selectedStoreUUID]: {
          ...
          storeMenu: Object.fromEntries(
            Object.entries(storesState[selectedStoreUUID].storeMenu).filter(
              ([drinkId]) => drinkId !== selectedDrinkUUID
            )
          ),
        },
      });
  ...
  return (
    <div className="flex">
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          ...
          >
            {dropdownValue
              ? frameworks.find(
                  (framework) => framework.label === dropdownValue.label
                )?.label
              : "Select drink..."}
                ...
                {frameworks.map((framework) => (
                  <CommandItem
                    ...
                    onSelect={(currentValue) => {
                      ...
                      setDropdownValue({
                        ...
                        value:
                          frameworks.find(
                            (framework) => framework.label === currentValue
                          )?.value ?? "",
                      });
                      setSelectedDrinkUUID(
                        frameworks.find(
                  ...
                  >
                    <Check
                      className={cn(
                        "mr-2 h-4 w-4",
                        dropdownValue.label === framework.label
                          ? "opacity-100"
                          : "opacity-0"
                      )}
                    />
      ...
      <Button onClick={() => deleteDrinkFromStore()} className="bg-[#dc2626]">
